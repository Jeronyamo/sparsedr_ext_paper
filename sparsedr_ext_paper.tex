...
In 2018, edge sampling, a differentiable rendering method applicable to path tracing, was introduced \cite{EdgeSampling}. The proposed approach accounts for both primary and secondary visibility and, for the first time, enables correct differentiation with respect to arbitrary scene parameters. The study demonstrated that when differentiating rendering integral with respect to parameters defining object shape, the integrand becomes discontinuous, and the locations of these discontinuities depend on the aforementioned parameters. In such cases, the derivative of the integral is obtained using the Reynolds transport theorem:

% формула

\begin{equation}
    \begin{gathered}
        \frac{d}{d \theta} \int_{\Omega_\theta}{f(\omega;\theta) d\omega} = \int_{\Omega_{\theta}}{\frac{d}{d \theta} f(\omega;\theta) d\omega} + \int_{\partial \Omega_{\theta}}{(\partial_\theta \omega \cdot \textbf{n}) f(\omega;\theta) d\omega}
    \end{gathered}
\end{equation}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\textwidth]{Resources/SparseDR.pdf}
    \caption{Comparison of current differentiable physically-based SDF rendering methods (left) and SparseDR (right). \label{figSparseDR}}
\end{figure}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\textwidth]{Resources/Graphic9c1.eps}
    \caption{Extrapolation step scheme, yellow brick side is extrapolated from neighbours' sides (colored green). a) neighbour lookup; b) side values extrapolation from each neighbour. \label{figNeighborBricks}}
\end{figure}
...
Local redistancing is an optimized version of the redistancing step performed after each iteration of the method. The first optimization restricts distance recomputation to bricks that contain the surface. The second optimization assumes that distances within surface-containing bricks depend only on the surface within the same brick, without referencing distances from neighboring bricks. This assumption simplifies the implementation and improves data locality without substantial degradation of reconstruction quality. The simplified comparison of redistancing variants is shown in Fig. \ref{figRedistTypes}.
...
% Реконструкция велась на 24 ракурсах, равномерно распределённых на сфере, разрешение изображений 1024*1024. В сцене два направленных источника света: (1,1,1) и (-1,-1,-1), а также ambient light. Первые 3 эпохи занимают по 200 итераций, оставшиеся 4 по 100 итераций. Размер батча 5. Hyperparameters, such as learning rate, Adam optimizer parameters, relaxed boundary width (epsilon), взяты из конфигурационного файла в репозитории \cite{}. Восстанавливались 4 модели from Stanford 3D scans repository, Nefertiti scan, Utah teapot, а также сканы из Artec3D: !--- TODO.
...
% ! Графики времени: 2 барплота, среднее по всем моделям время выполнения каждой эпохи. 1 - сравнение нас и базового метода (5 эпох). 2 - для нашего метода (7 эпох).
...
% ! Графики качества: 1. базовый + наш (та же эпоха); 2. базовый + наш (то же время)
...
% 1. Скорость реконструкции (только этап рендеринга) с sphere tracing vs. Newton/Analytic -- насколько второй быстрее (скорость, барплот по эпохам, два столбца)

% 2. Реконструкция с обеими версиями редистансинга -- скорость (без графика, только во сколько раз быстрее на CPU), качество (качество, барплот по моделям, два столбца)

% Качество реконструкции (качество, барплот по моделям, два столбца - исходные ракурсы + новые)



% 3. Демонстрация качества (барплот по эпохам) для каждой эпохи -- PSNR растёт линейно
